import { copyFileSync, unlinkSync, readFileSync, writeFileSync, existsSync } from 'fs'
...
if (existsSync('./zig-out/lib/zigWASM.wasm')) {
  copyFileSync('./zig-out/lib/zigWASM.wasm', './dist/zigWASM.wasm')
} else {
  writeFileSync('./dist/zigWASM.wasm', Buffer.from([]))
}
if (existsSync('./zig-out/lib/zigWASI.wasm')) {
  copyFileSync('./zig-out/lib/zigWASI.wasm', './dist/zigWASI.wasm')
} else {
  writeFileSync('./dist/zigWASI.wasm', Buffer.from([]))
}
copyFileSync('./workers-zig/memfs.wasm', './dist/memfs.wasm')

// step 3: final build
esbuild
  .build({
    entryPoints: ['./dist/tmp_worker.mjs'],
    format: 'esm',
    sourcemap: true,
    treeShaking: true,
    bundle: true,
